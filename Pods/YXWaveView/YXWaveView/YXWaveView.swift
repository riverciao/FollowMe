...
open class YXWaveView: UIView {
    ...
    /**
     ...
     - parameter frame: view frame
     - parameter color: water wave color
     
     - returns: view
     */
    public convenience init(frame: CGRect, waterColor:UIColor) {
        self.init(frame: frame)
        
        self.maskWaveColor = waterColor
        var hue: CGFloat = 0
        var saturation: CGFloat = 0
        var brightness: CGFloat = 0
        var alpha: CGFloat = 0
        
        waterColor.getHue(&hue, saturation: &saturation, brightness: &brightness, alpha: &alpha)
        
        let waveColor = UIColor(hue: hue, saturation: saturation - 0.8, brightness: brightness, alpha: alpha)
        self.realWaveColor = waveColor.withAlphaComponent(0.4)
